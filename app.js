...
const UI = {
 ...
 spinner(size = 20) {
 return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="spinner"><circle cx="12" cy="12" r="10" opacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10" opacity="0.75"/></svg>`;
 },
...
const SIM = {
 ...
 const res = await Utils.fetchWithRetry(`${API}/sim/new`, { method: 'POST' });
...
const WS = {
 ...
 if (!net_number || !State.deviceId) {
 console.error('[WS] Cannot connect: missing net_number or device_id');
 return;
 }
 ...
 console.warn('[WS] not ready, queueing message', obj.type);
 State.msgQueue.push(obj);
 if (!State.wsConnecting) WS.connect();
 }
 },
 ...
 case 'call_offer':
 if (msg.room) GroupCall.handleOffer(msg);
 else Call.handleOffer(msg);
 break;

 case 'call_answer':
 if (msg.room) GroupCall.handleAnswer(msg);
 else Call.handleAnswer(msg);
 break;
 ...
 case 'ice_candidate':
 if (msg.room) GroupCall.handleIce(msg);
 else Call.handleIce(msg);
 break;
...
const Status = {
 ...
 try {
 const res = await Utils.fetchWithRetry(`${API}/status/batch`, {
 method: 'POST',
 headers: { 'Content-Type': 'application/json' },
...
const Render = {
 ...
 el.innerHTML = `
 <div class="contact-avatar" style="background:${Utils.avatarColor(c.pin)}">${Utils.initials(c.name)}</div>
 ...
 const badge = msg.type === 'api' ? `<span class="api-badge">via API</span>` : '';
 bubbleHtml = `
 <div class="bubble">
 ${badge}${linkify(msg.content)}
 ...
 <img class="msg-img" src="${msg.media || ''}" alt="photo" loading="lazy" />
 ${msg.caption ? `<div class="caption">${linkify(msg.caption)}</div>` : ''}
 <div class="msg-meta">${time}${tick}</div>
 </div>`;
 }
 ...
 bubbleHtml = `
 <div class="bubble media-bubble">
 <video class="msg-video" src="${msg.media || ''}" preload="metadata" playsinline></video>
 ${msg.caption ? `<div class="caption">${linkify(msg.caption)}</div>` : ''}
 <div class="msg-meta">${time}${tick}</div>
 </div>`;
 }
 ...
 const barHtml = bars.map(h => `<div class="bar" style="height:${h}px"></div>`).join('');
 bubbleHtml = `
 <div class="bubble voice-bubble" data-src="${msg.media || ''}">
 <div class="voice-row">
 <button class="voice-play" type="button">▶</button>
 <div class="waveform">${barHtml}</div>
 <span class="voice-dur">${msg.content || '0:00'}</span>
 </div>
 <div class="msg-meta">${time}${tick}</div>
 </div>`;
 }
 else if (msg.type === 'sticker') {
 bubbleHtml = `<div class="bubble sticker-bubble"><div class="sticker-lg">${msg.content}</div></div>`;
 }
 else if (msg.type === 'file') {
 bubbleHtml = `
 <div class="bubble file-bubble" data-src="${msg.media || ''}">
 <button class="file-download" type="button">📎</button>
 <span class="file-name">${msg.content || 'File'}</span>
 <div class="msg-meta">${time}${tick}</div>
 </div>`;
 }
 ...
 const vid = wrap.querySelector('.msg-video');
 if (vid) {
 vid.onclick = (e) => {
 e.preventDefault();
 ...
 if (msg.type === 'voice' && msg.media) {
 const btn = wrap.querySelector('.voice-play');
 if (btn) btn.onclick = () => App.playVoiceNote(btn, msg.media);
 }
 if (msg.type === 'file' && msg.media) {
 const btn = wrap.querySelector('.file-download');
 if (btn) {
 btn.onclick = () => {
 const a = document.createElement('a');
 a.href = msg.media;
 a.download = msg.content || 'download';
 a.click();
 };
 }
 }
 ...
 el.innerHTML = `
 <div class="call-icon ${typeClass}">${typeIcon}</div>
 <div class="call-info">
 <div class="call-name">${name}</div>
 <div class="call-meta">${log.type} · ${Utils.formatDuration(log.duration)} · ${Utils.formatTime(log.ts)}</div>
 </div>
 `;
 list.appendChild(el);
 });
 ...
 item.innerHTML = `
 <div class="call-icon outgoing"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg></div>
 <div class="call-info">
 <div class="call-name">${name}</div>
 <div class="call-meta">${log.pin} · ${Utils.formatTime(log.ts)}</div>
 </div>
 `;
 el.appendChild(item);
 });
 ...
 <p>No rooms yet</p>
 <p>Create a room and share the link to start a group call</p>
 </div>`;
 return;
 }
 rooms.forEach(room => {
 ...
 <div class="room-when">${when}</div>
 <div class="room-actions">
 <button class="btn-sm" onclick="App.joinRoom('${room.id}','${room.name.replace(/'/g,"\\'")}')">Join</button>
 <button class="btn-sm ghost" onclick="App.shareRoom('${room.id}','${room.name.replace(/'/g,"\\'")}')">Share</button>
 </div>
 ...
 <div class="contact-avatar" style="background:${Utils.avatarColor(c.pin)}">${Utils.initials(c.name)}</div>
 <div class="contact-info"><div class="contact-name">${c.name}</div><div class="contact-pin">${c.pin}</div></div>
 <div class="invite-check" data-pin="${c.pin}" onclick="this.classList.toggle('checked')"></div>
 `;
 el.appendChild(div);
 });
 },
 ...
 selfDiv.id = 'gc-self';
 selfDiv.innerHTML = `
 <video class="gc-video" id="gc-local-video" autoplay muted playsinline></video>
 <div class="gc-avatar" style="background:${Utils.avatarColor(State.sim.net_number)}">${Utils.initials(State.sim.net_number)}</div>
 <div class="gc-name">You</div>
 <div class="gc-status">You</div>
 `;
 ...
 div.id = `gc-p-${pin}`;
 div.innerHTML = `
 <video class="gc-video" id="gc-video-${pin}" autoplay playsinline></video>
 <div class="gc-avatar" style="background:${Utils.avatarColor(pin)}">${Utils.initials(name)}</div>
 <div class="gc-name">${name}</div>
 <div class="gc-status">${name} ${member.muted ? '<span class="muted">Muted</span>' : ''}</div>
 `;
 el.appendChild(div);
 });
 },
...
const ICE_CONFIG = {
 ...
 // Open Relay Project (free, verified working — 20GB/month)
 { urls: 'turn:openrelay.metered.ca:80', username: 'openrelayproject', credential: 'openrelayproject' },
 { urls: 'turn:openrelay.metered.ca:443', username: 'openrelayproject', credential: 'openrelayproject' },
 { urls: 'turn:openrelay.metered.ca:443?transport=tcp', username: 'openrelayproject', credential: 'openrelayproject' },
 { urls: 'turns:openrelay.metered.ca:443?transport=tcp', username: 'openrelayproject', credential: 'openrelayproject' },

 // numb.viagenie.ca (free, long-standing public TURN)
 ...
 // turn.anyfirewall.com (free, works through strict firewalls)
 { urls: 'turn:turn.anyfirewall.com:443?transport=tcp', username: 'webrtc', credential: 'webrtc' },

 // Additional public STUN fallbacks
...
async function gatherIce(pc, timeoutMs = 2500) {
...
const Call = {
 ...
 await gatherIce(State.peerConn);

 WS.send({
 type: 'call_offer',
 target: pin,
 call_type: type,
 sdp: State.peerConn.localDescription.sdp
 });
 },
 ...
 ? `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg> Video Call`
 ...
 WS.send({ type: 'call_answer', target: msg.from, sdp: State.peerConn.localDescription.sdp });
 },
...
const GroupCall = {
 ...
 WS.send({ type: 'call_offer', target: pin, sdp: pc.localDescription.sdp, call_type: 'video', room: State.groupCall.roomId });
 },
 ...
 WS.send({ type: 'call_answer', target: pin, sdp: pc.localDescription.sdp, room: State.groupCall.roomId });
 },

 async handleAnswer(msg) {
 if (!State.groupCall) return;
 const pc = State.groupCall.peerConns[msg.from];
 if (!pc) return;
...
const App = {
 ...
 console.error('Create SIM error:', e);
 UI.toast('Error', e.message || 'Could not create SIM. Please try again.');
 } finally {
 UI.setLoading('create-sim-btn', false);
 UI.$('create-sim-btn').innerHTML = `Create SIM`;
 }
 },
 ...
 let simData;
 try {
 simData = await SIM.decryptFile(State._pendingSimData, pass);
 } catch (e) {
 UI.toast('Wrong password', 'Could not decrypt SIM file');
 return;
 }

 if (!simData?.net_number) {
 UI.toast('Invalid SIM', 'This file is not a valid SIM');
 return;
 }
 ...
 UI.$('unlock-btn').innerHTML = `Unlock & Connect`;
 }
 },
 ...
 await Utils.fetchWithRetry(`${API}/sim/register-fingerprint`, {
 method: 'POST',
 headers: { 'Content-Type': 'application/json' },
 ...
 audio.onended = () => { btn.innerHTML = svgPlay; btn._audio = null; };
