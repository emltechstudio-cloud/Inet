...
/* ══════════════════════════════════════════════════
   iNet v2 · app.js  (FIXED VERSION)
   EML Tech Studio

   BUG FIXES APPLIED:
   1. Fixed CRITICAL syntax error in Render object (tabBadges code was outside function)
   2. Added proper Render.tabBadges() function definition
   3. Fixed State.simPassword not restored from sessionStorage
   4. Added WS connection status UI feedback
   5. Added message queue for offline sending
   6. Fixed error handling in unlockSim() to distinguish password vs network errors
   7. Added retry logic for failed API calls
   8. Added TURN server placeholder for WebRTC NAT traversal
   9. Fixed group call ICE routing to include room context
   10. Added connection state handling for group call peers
══════════════════════════════════════════════════ */
...
const ICE_CONFIG = {
  iceServers: [
    ...
    // Open Relay Project — free TURN servers (no signup required)
    {
    ...
    // Numb TURN server (free account required but publicly known credentials)
    {
